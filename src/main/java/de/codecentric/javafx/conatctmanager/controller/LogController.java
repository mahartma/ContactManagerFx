...
public class LogController {
    ...
    public void initialize() {
        ...
        items.addAll(repository.readAll().stream()
                .map(LogEntry::toString)
                .collect(toList()));

        ObservableList<Contact> data = contactRepository.getData();
        data.addListener((ListChangeListener.Change<? extends Contact> change) -> {
            handleListChange(change);
        });
        data.forEach(contact -> contact.emailProperty().addListener((changedEmail, oldEmail, newEmail) -> {
